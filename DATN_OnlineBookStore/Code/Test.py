    ...
    try:
        ...
        if rating_count > 0:
            ...
            if str(user_id) in user_item_matrix.index:
                ...
                final_recommendations = pd.DataFrame(columns=['ItemId', 'CollaborativeScore'])
            
            recommendations_list = final_recommendations.rename(columns={'ItemId': 'ItemId', 'CollaborativeScore': 'Score'}).to_dict(orient='records')
            ...
            if item_profiles is not None and user_info is not None:
                ...
                recommendations_list = similar_items.sort_values(by='ContentBasedScore', ascending=False).head(n_recommendations).rename(columns={'PK_iSanphamID': 'ItemId', 'ContentBasedScore': 'Score'}).to_dict(orient='records')
            else:
                recommendations_list = []

    except Exception as e:
        print(f"Error in generating recommendations: {e}")
        return json.dumps({'error': f'Error in generating recommendations: {e}'})
    return json.dumps(recommendations_list)

@app.route('/api/recommendation/user/<int:user_id>/recommendations', methods=['GET'])
def recommend(user_id):
    n_recommendations = request.args.get('n', 5)
    try:
        recommendations = recommend_items(user_id, int(n_recommendations))
        response_json = json.loads(recommendations)
    except Exception as e:
        print(f"Error in the recommendation endpoint: {e}")
        response_json = {'error': 'Failed to generate recommendations'}
